...
namespace RimboundCore.HarmonyPatches
{
    ...
    public static class HarmonyPatch_HeridityPatches
    {
        ...
        private static XenotypeDef inheritedXenotype;
        ...
        static HarmonyPatch_HeridityPatches()
        {
            ...
            harmony.Patch(
                AccessTools.Method(typeof(PregnancyUtility), nameof(PregnancyUtility.GetInheritedGenes), new Type[]
                {
                    typeof(Pawn),
                    typeof(Pawn),
        ...
        {
            if ((father?.RaceProps?.Humanlike == true || mother?.RaceProps?.Humanlike == true) && (father?.genes != null || mother?.genes != null))
            {
                List<Gene> genesA = mother.genes.GenesListForReading;
                List<Gene> genesB = father.genes.GenesListForReading;

                GeneInheritanceExtension extensionA = CheckForModExtension(genesA);
                GeneInheritanceExtension extensionB = CheckForModExtension(genesB);
                ...
                if (extensionA != null || extensionB != null)
                {
                    ...
                    {
                        List<GeneDef> list1 = new List<GeneDef>();
